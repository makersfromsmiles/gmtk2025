...
func _physics_process(delta: float) -> void:
	...
	if is_throw_beginning:
		...
		if position.distance_to(lasso_point.global_position) < 2.0:
			position = lasso_point.global_position
			velocity = Vector2.ZERO
			is_returning = false
			visible = false
		else:
			var return_velocity : Vector2 = (lasso_point.global_position - position)
			velocity = (return_velocity.normalized() * 50.0 * speed * delta)
			move_and_slide()

	elif is_connecting:
		if position.distance_to(connected_critter.global_position) < 2.0:
